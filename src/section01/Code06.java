...
public class Code06 {
	...
	static Polynomial [] polys = new Polynomial [100];   // 다항식의 최대개수:100 최대개수 넘어버리면 생기는 문제는 나중에 다룸
		...
		while (true) {
			...
			if (command.equals("create")) {
				...
				if (index==-1) {
					...
					int c = kb.nextInt();
					int e = kb.nextInt();
					addTerm(polys[index],c,e); // 다항식 객체에 대한 참조를 넘겨줌
				}
				...
				if (index==-1) {
					...
					int x = kb.nextInt();
					calPolynomial(polys[index],x); // 다항식 객체에 대한 참조를 넘겨줌
				}
			}
	...
	private static void addTerm(Polynomial p, int c, int e) {  // 다항식 p
		int index = findTerm(p,e);
		if (index != -1) {
			p.terms[index].coef += c;
		}
		else {
			int i =p.nterms-1; // i 다항식 인덱스
